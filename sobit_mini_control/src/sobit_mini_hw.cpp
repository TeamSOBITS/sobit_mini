...
SobitMiniControl::SobitMiniControl()
{
  ...
  hardware_interface::JointStateHandle state_handle_right_shoulder_roll  ("right_shoulder_roll_joint",     &pos_[4], &vel_[4], &eff_[4]);
  hardware_interface::JointStateHandle state_handle_right_shoulder_flex  ("right_shoulder_flex_joint",     &pos_[5], &vel_[5], &eff_[5]);
  hardware_interface::JointStateHandle state_handle_right_wrist_flex  ("right_wrist_flex_joint",     &pos_[6], &vel_[6], &eff_[6]);
  hardware_interface::JointStateHandle state_handle_right_hand_motor  ("right_hand_motor_joint",     &pos_[7], &vel_[7], &eff_[7]);
  hardware_interface::JointStateHandle state_handle_left_shoulder_roll  ("left_shoulder_roll_joint",     &pos_[8], &vel_[8], &eff_[8]);
  hardware_interface::JointStateHandle state_handle_left_shoulder_flex  ("left_shoulder_flex_joint",     &pos_[9], &vel_[9], &eff_[9]);
  hardware_interface::JointStateHandle state_handle_left_wrist_flex  ("left_wrist_flex_joint",     &pos_[10], &vel_[10], &eff_[10]);
  hardware_interface::JointStateHandle state_handle_left_hand_motor  ("left_hand_motor_joint",     &pos_[11], &vel_[11], &eff_[11]);
  ...
  hardware_interface::JointHandle pos_handle_right_shoulder_roll  (jnt_state_interface_.getHandle("right_shoulder_roll_joint"),     &cmd_[4]);
  hardware_interface::JointHandle pos_handle_right_shoulder_flex  (jnt_state_interface_.getHandle("right_shoulder_flex_joint"),     &cmd_[5]);
  hardware_interface::JointHandle pos_handle_right_wrist_flex    (jnt_state_interface_.getHandle("right_wrist_flex_joint"),       &cmd_[6]);
  hardware_interface::JointHandle pos_handle_right_hand_motor    (jnt_state_interface_.getHandle("right_hand_motor_joint"),       &cmd_[7]);
  hardware_interface::JointHandle pos_handle_left_shoulder_roll  (jnt_state_interface_.getHandle("left_shoulder_roll_joint"),     &cmd_[8]);
  hardware_interface::JointHandle pos_handle_left_shoulder_flex  (jnt_state_interface_.getHandle("left_shoulder_flex_joint"),     &cmd_[9]);
  hardware_interface::JointHandle pos_handle_left_wrist_flex  (jnt_state_interface_.getHandle("left_wrist_flex_joint"),     &cmd_[10]);
  hardware_interface::JointHandle pos_handle_left_hand_motor  (jnt_state_interface_.getHandle("left_hand_motor_joint"),     &cmd_[11]);
  ...
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_body_lift  (pos_handle_body_lift,   joint_limits, soft_joint_limits);
  getJointLimits("body_roll_joint",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_body_roll  (pos_handle_body_roll,   joint_limits, soft_joint_limits);
  getJointLimits("head_tilt_joint",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_head_tilt(pos_handle_head_tilt, joint_limits, soft_joint_limits);
  getJointLimits("head_pan_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_head_pan(pos_handle_head_pan, joint_limits, soft_joint_limits);
  getJointLimits("right_shoulder_roll_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_right_shoulder_roll(pos_handle_right_shoulder_roll, joint_limits, soft_joint_limits);
  getJointLimits("right_shoulder_flex_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_right_shoulder_flex(pos_handle_right_shoulder_flex, joint_limits, soft_joint_limits);
  getJointLimits("right_wrist_flex_joint",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_right_wrist_flex  (pos_handle_right_wrist_flex,   joint_limits, soft_joint_limits);
  getJointLimits("right_hand_motor",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_right_hand_motor  (pos_handle_right_hand_motor,   joint_limits, soft_joint_limits);
  getJointLimits("left_shoulder_roll_joint",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_left_shoulder_roll(pos_handle_left_shoulder_roll, joint_limits, soft_joint_limits);
  getJointLimits("left_shoulder_flex", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_left_shoulder_flex(pos_handle_left_shoulder_flex, joint_limits, soft_joint_limits);
  getJointLimits("left_wrist_flex", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_left_wrist_flex(pos_handle_left_wrist_flex, joint_limits, soft_joint_limits);
  getJointLimits("left_hand_motor", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_left_hand_motor(pos_handle_left_hand_motor, joint_limits, soft_joint_limits);
...
void SobitMiniControl::read(ros::Time time, ros::Duration period)
{
  ...
  {
    if(     pose.name[i] == "body_lift_joint")         {pos_[0] = pose.position[i]; }
    else if(pose.name[i] == "body_roll_joint")         {pos_[1] = pose.position[i]; }
    else if(pose.name[i] == "head_tilt_joint")         {pos_[2] = pose.position[i]; }
    else if(pose.name[i] == "head_pan_joint")         {pos_[3] = pose.position[i]; }
    else if(pose.name[i] == "right_shoulder_roll_joint")         {pos_[4] = pose.position[i]; }
    else if(pose.name[i] == "right_shoulder_flex_joint")         {pos_[5] = pose.position[i]; }
    else if(pose.name[i] == "right_wrist_flex_joint")         {pos_[6] = pose.position[i]; }
    else if(pose.name[i] == "right_hand_motor_joint")         {pos_[7] = pose.position[i]; }
    else if(pose.name[i] == "left_shoulder_roll_joint")         {pos_[8] = pose.position[i]; }
    else if(pose.name[i] == "left_shoulder_flex_joint")         {pos_[9] = pose.position[i]; }
    else if(pose.name[i] == "left_wrist_flex_joint")         {pos_[10] = pose.position[i]; }
    else if(pose.name[i] == "left_hand_motor_joint")         {pos_[11] = pose.position[i]; }
  }
}

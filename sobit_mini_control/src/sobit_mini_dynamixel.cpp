...
void SobitMiniDynamixel::writeDynamixelMotors(const trajectory_msgs::JointTrajectory &pose)
{
  for(int i = 0; i < pose.joint_names.size(); i++)
  {
    ...
    if (std::abs(joint_pos == saved_dxl_goal_position[joint_id])) { continue; }
...
float SobitMiniDynamixel::toBit(int id, float rad)
{
  ...
  {
    return 2048 - (rad * 5258 / M_PI_2); // 2048は胴が正面の状態の値,5258は胴が正面を向いている状態から左(-3210)・右(7306)向きの状態を引いた値, 1.57は正面の状態から左・右の向きに向いた時のrad差
  }
  else if (id == 3)
  {
    return 2250 - (rad * 750 / 0.5); // 2250は顔が前向きの状態の値,750は顔が前向きの状態から下(1500)・上(3000)向きの状態を引いた値, 0.5は前向き状態から下・上の向きに向いた時のrad差
  }
  else if (id == 4)
  {
    return 3140 + (rad * 1460 / 2.62); // 3140は顔がまっすぐの状態の値, 1460は顔がまっすぐの状態から左(4600)・右(1680)の状態を引いた値、2.62はまっすぐの状態から左・右の向きに向いた時のrad差
  }
  else if (id == 10)
  ...
  {
    return 2200 - (rad * 1000 / M_PI_2); // 2200は閉めた状態　1000は開きの状態(1200)から閉めの状態(2200)を引いた値, 1.57は閉めから開き状態の時のrad差
  }
  else if (id == 20)
    return 2048 - (rad / -0.001533203125);
  else if (id == 21)
  ...
  {
    return 1900 + (rad * 1000 / M_PI_2); // 1900は閉めた状態  1000は開きの状態(2900)から閉めの状態(1900)を引いた値, 1.57は閉めから開き状態の時のrad差
  }
  else
    return -1;
}
...
{
  if (joint_name == "body_lift_joint") { return (bit - 2048) / 90163.9344262; }

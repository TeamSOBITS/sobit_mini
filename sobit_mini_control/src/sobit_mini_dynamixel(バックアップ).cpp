...
SobitMiniDynamixel::SobitMiniDynamixel() {
  ...
  std::ofstream ofs("/home/kento-nasu/catkin_ws/src/sobit_mini/sobit_mini_control/config/debug_read.csv");
  ofs << "ID,"
      << "joint_name,"
      << "dynamixel_pos,"
      << "joint_pos" << std::endl;
  std::ofstream ofs_write("/home/kento-nasu/catkin_ws/src/sobit_mini/sobit_mini_control/config/debug_write.csv");
  ofs_write << "ID,"
            << "joint_name,"
            << "dynamixel_pos,"
            << "joint_pos" << std::endl;
...
void SobitMiniDynamixel::writeDynamixelMotors(const trajectory_msgs::JointTrajectory &pose) {
  ...
  for (int i = 0; i < pose.joint_names.size(); i++) {
    ...
    if (joint_id == 12) {
      ofs_write << joint_id << "," << pose.joint_names[i] << "," << joint_pos << "," << joint_pos << std::endl;
    }
    if (joint_id == 21) {

...
: Node("wheel_action_server", options)
{
  ...
  qos_profile.reliability(RMW_QOS_POLICY_RELIABILITY_RELIABLE); // Turtlebot2 is 'RELIABLE'
  ...
  const std::shared_ptr<GoalHandleMoveWheelLinear> goal_handle)
{
  ...
  while (curt_dist < goal_dist) {
    ...
    feedback->current_point.x = curt_dist * (goal->target_point.x/std::abs(goal->target_point.x));
